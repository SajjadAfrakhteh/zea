...
class CoherenceFactor(Operation):
    ...
    .. math::
        ...
        CF = \\frac{|\\sum_{i=1}^{N} x_i|^2}{N \\sum_{i=1}^{N} |x_i|^2}

    The CF ranges from 0 (completely incoherent) to 1 (perfectly coherent).
    The beamformed output is generated by multiplying the standard DAS sum
    by the calculated CF.

    **Citation:**

    * Hollman, K. W., Rigby, K. W., & O'Donnell, M. (1999). Coherence factor
      of speckle from a multi-row probe. IEEE Ultrasonics Symposium.
    ...
    def process_image(self, data):
        # data shape: (..., n_tx, n_pixels, n_el, n_ch)

        # ---- Step 1: DAS (per transmit) ----
        # Sum over elements (n_el) -> (..., n_tx, n_pixels, n_ch)
        das_per_tx = ops.sum(data, axis=-2)

        # ---- Step 2: Compute Coherence Factor ----
        # CF = |sum(x)|^2 / (N * sum(|x|^2))
        # To handle IQ (n_ch=2) and RF (n_ch=1) consistently:

        # 2a. Coherent Power (Numerator): |sum(elements)|^2
        # If IQ, this is I^2 + Q^2 of the summed signal
        coherent_power = ops.sum(ops.square(das_per_tx), axis=-1, keepdims=True)

        # 2b. Incoherent Power (Denominator): sum(|elements|^2)
        # We square every element and channel first, then sum over elements
        total_element_power = ops.sum(ops.square(data), axis=-1)  # (..., n_tx, n_pixels, n_el)
        incoherent_power = ops.sum(
            total_element_power, axis=-2, keepdims=True
        )  # (..., n_tx, n_pixels, 1)

        # 2c. CF Weight
        # Weight shape: (..., n_tx, n_pixels, 1)
        cf_weight = coherent_power / (incoherent_power + 1e-12)

        # ---- Step 3: Apply Weighting ----
        # Apply CF per transmit (broadcasts across n_ch)
        weighted_data = das_per_tx * cf_weight

        # ---- Step 4: Compounding (Sum over transmits) ----
        # n_tx is axis -3 in (..., n_tx, n_pixels, n_ch)
        return ops.sum(weighted_data, axis=-3)
...
class GeneralizedCoherenceFactor(Operation):
    ...
    delayed channel data across :math:`N` elements. The GCF is calculated as:

    .. math::

        GCF = \\frac{\\sum_{k \\in \\mathcal{M}_0} |A(k)|^2}{\\sum_{k=0}^{N-1} |A(k)|^2}

    where :math:`\\mathcal{M}_0` is the set of indices representing the low spatial
    frequency region defined by the parameter :math:`m_0`.

    **Citation:**

    * Li, P. C., & Li, M. L. (2003). Adaptive imaging using the generalized
      coherence factor. IEEE Transactions on Ultrasonics, Ferroelectrics,
      and Frequency Control, 50(2), 128-141.

    Args:
        m0 (int): Cutoff frequency index for the low-frequency spatial region.
            Higher values increase tolerance to phase aberrations. Default is 4.
        **kwargs: Additional arguments passed to the Operation base class.
    ...
    def process_image(self, data):
        ...
        # 1. Prepare for FFT: Move n_el to the last axis
        # (..., n_tx, n_pixels, n_el, n_ch) -> (..., n_tx, n_pixels, n_ch, n_el)
        spatial_data = ops.moveaxis(data, -2, -1)

        # 2. Extract Real and Imaginary
        # If n_ch=2, it's IQ. If n_ch=1, imag becomes 0.
        real_in = spatial_data[..., 0, :]

        # We use a logical mask to handle the imaginary part.
        # If n_ch > 1, take index 1. Otherwise, zeros.
        def _get_imag():
            # Only called if n_ch == 2
            return spatial_data[..., 1, :]

        def _get_zeros():
            # Only called if n_ch == 1
            return ops.zeros_like(real_in)

        imag_in = ops.cond(ops.equal(n_ch, 2), _get_imag, _get_zeros)

        # 3. Spatial FFT & Power Spectrum
        real_fft, imag_fft = ops.fft((real_in, imag_in))
        power_spectrum = ops.square(real_fft) + ops.square(imag_fft)

        # 4. GCF Weight Calculation
        total_energy = ops.sum(power_spectrum, axis=-1, keepdims=True)

        # Frequency mask
        indices = ops.arange(n_el)
        low_freq_mask = ops.logical_or(
            ops.less_equal(indices, self.m0),
            ops.greater_equal(indices, n_el - self.m0),
        )

        # Sum low-frequency bins
        ...
        # Weight shape: (..., n_tx, n_pixels, 1)
        gcf_weight = low_freq_energy / (total_energy + 1e-12)

        # 5. DAS + Weighting + Compounding
        # Sum over elements (n_el): (..., n_tx, n_pixels, n_ch)
        das_per_tx = ops.sum(data, axis=-2)

        # Apply weight per transmit (broadcasts across n_ch)
        # Note: gcf_weight is (..., n_tx, n_pixels, 1)
        # das_per_tx is (..., n_tx, n_pixels, n_ch)
        weighted_data = das_per_tx * gcf_weight

        # Compound: Sum over transmits (n_tx is axis -3)
        return ops.sum(weighted_data, axis=-3)
